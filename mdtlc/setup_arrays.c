...
// some kind of bmp file interface stuff here? (each line multiple of 4 bytes?)
//	 offset+=3;
...
//          }
//       }
           cell_dielectric[w][height-1-h]= 255;  // 255 = not on command line - mdtlc
            for(i=0;i<data->dielectrics_on_command_line;++i)
            {
	         if (Er_on_command_line[i].other_colour ==  colour_mixture)
	         {
	          ...
	          cell_dielectric[w][height-1-h]= i;
	          dielectric_found=TRUE;
         ...
         {
     		  printf("diel %x %x\n",colour_mixture ,Er_on_command_line[i].other_colour );
              system("PAUSE");
   ...
   // added for mdtlc to keep red and blue pixel count information
   data->pixels_positive = pixels_found.red;
   data->pixels_negative = pixels_found.blue;
      // added for mdtlc to keep red and blue pixel count information
