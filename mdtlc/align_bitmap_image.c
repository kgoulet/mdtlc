...
// For mdtlc - do not see the image reversal here - maybe it's done elsewhere
...
//   int i, j, vector_aligned=0, vector_original=0;
   int i, j, vector_aligned=0;
   int vector_original=3*W*H-3;   // pointing to the end of the original and working backwards
   ...
   for(j=0;j<H;j++)
   {
	 ...
	 if( (i==0) && (vector_aligned%4!=0) )
	    byte_aligned_image[vector_aligned++]=0;
	    //vector_aligned++;
	 if( (i==0) && (vector_aligned%4!=0) )
	    //vector_aligned++;
  	    byte_aligned_image[vector_aligned++]=0;
	 if( (i==0) && (vector_aligned%4!=0) )
	    byte_aligned_image[vector_aligned++]=0;
	    //vector_aligned++; 
	 byte_aligned_image[vector_aligned]=unaligned_image[vector_original+2];
	 byte_aligned_image[vector_aligned+1]=unaligned_image[vector_original+1];
	 byte_aligned_image[vector_aligned+2]=unaligned_image[vector_original];
         vector_original-=3; // counting down mdtlc change
         vector_aligned+=3;
      }
   }
   if( vector_aligned%4!=0 ) 	    byte_aligned_image[vector_aligned++]=0;
//      vector_aligned++;
   if( vector_aligned%4!=0 )	    byte_aligned_image[vector_aligned++]=0;
 //     vector_aligned++;
   if( vector_aligned%4!=0 )	    byte_aligned_image[vector_aligned++]=0;
