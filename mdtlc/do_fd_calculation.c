...
void do_fd_calculation(struct transmission_line_properties *data, size_t size, FILE *where_to_print_fp, char *inputfile_filename)
{
  ...
  int count=0;
  int w,h; // added for mdtlc -- note that w and h are used many times in atlc
  double* pDouble;
  // stuff added for mdtlc loss tangent
  int i;
  double nrg, total_nrg;
  double tangent_loss_nrg;
...
// added for mdtlc - dielectric loss calculation
// sum the total energy and the energy in each dielectric
  ...
  for(h=(height-1);h>=0;h--)
    {
      for(w=0;w<width;++w)
      {
        if ( cell_type[w][h]==DIELECTRIC ) {
             nrg = find_energy_per_metre(w,h);
             if ( cell_dielectric[w][h] < 255 ) {
                tangent_loss_nrg += Er_on_command_line[cell_dielectric[w][h]].loss_tangent*nrg;
                }
             else {
                 tangent_loss_nrg += nrg;
                 }
             total_nrg += nrg;
             }  
       }
    }
    data->loss_tan_o = tangent_loss_nrg/total_nrg;    
    //
// added for mdtlc to plot field information
//
  pDouble = pVodd;
...
// sum the total energy and the energy in each dielectric
  ...
  for(h=(height-1);h>=0;h--)
    ...
    data->loss_tan_e = tangent_loss_nrg/total_nrg;

//      if(data->write_binary_field_imagesQ == TRUE || data->write_bitmap_field_imagesQ == TRUE)
//        write_fields_for_directional_couplers(inputfile_filename, *data, size, EVEN);
    } /* end of stage 4 */
    /* Print the results if the verbose level was 0 (no -v flag(s) ). */
    data->verbose_level = 0; // turn off verbose to force level 1
    if (data->verbose_level == 0)
    {
